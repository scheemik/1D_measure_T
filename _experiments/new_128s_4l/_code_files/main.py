...
dt^2[dz^2(psi) - k^2 psi] - k^2 N^2 psi + f_0 dz^2(psi) - nu[dz^4(psi) - 2*dz^2(k^2 psi) + k^4 psi] = 0
...
problem.add_equation("dt( dz(dz(foo)) - (k**2)*foo ) + f0*(dz(dz(psi))) " \
                     " - NU*(dz(dz(dz(dz(psi)))) - 2*(dz(dz((k**2)*psi))) + (k**4)*psi) " \
                     " = (k**2)*((N0*BP)**2)*psi " \
...
# Add analysis task for momentum flux
snapshots.add_task("-(dz(psi))*(k*psi)", layout='g', name='J_p')
