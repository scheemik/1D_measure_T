...
def plot_T_vs_kL(all_exp_data, theta, omega, title_str='Transmission Coefficient', filename='pres_64s_T_vs_kL.png'):
    ...
    for i in range(1,all_exp_data.shape[1]):
        axes.plot(all_exp_data[:,0], all_exp_data[:,i], color=plt_clrs[i-1], marker=markers[i-1], markersize=5, label=labels[i-1])#rf'$T_{i}$')
    axes.legend(fontsize=this_font_size-2)
    # Add labels and titles
    axes.set_xlabel(r'$k_xL$', fontsize=this_font_size)
    axes.set_xlim(0,2.5)
    axes.set_ylabel(r'T', fontsize=this_font_size+6)
